...
// Official brand marks (path data + hex from simple-icons, CC0) so the checkout
// strip shows accurate logos instead of CSS-approximated text.
const paymentBadges = [
  {
    id: 'visa',
    label: 'Visa',
      ...
      {paymentBadges.map((payment) => (
        <span className={`payment-badge payment-badge--${payment.id}`} key={payment.id} aria-label={payment.label}>
          <span className="payment-badge-mark" aria-hidden="true">
            {payment.path && (
              <svg viewBox={payment.viewBox} fill={payment.color} xmlns="http://www.w3.org/2000/svg">
                <path d={payment.path} />
              </svg>
            )}
          </span>
...
function App() {
  ...
  useEffect(() => {
    ...
    updateStickyCart()
    // Product images can still be loading at first mount, so the very first
    // measurement can land on a too-short layout and flash the sticky bar on
    // briefly before correcting itself on the next scroll/resize. Re-check once
    // shortly after mount so that flash doesn't happen.
    const settleTimer = window.setTimeout(updateStickyCart, 250)
        ...
        {checkoutOpen ? (
          ...
          selectedProduct ? (
            ...
            >
              ...
              <div
                className={`pdp-mobile-cta${pdpStickyCartVisible ? ' is-visible' : ''}`}
                aria-hidden={!pdpStickyCartVisible}
              >
                <img src={activeProductImage} alt="" />
                <div className="pdp-mobile-cta-info">
                  <strong>{selectedProduct.name}</strong>
                  <span>{formatPrice(selectedVariantPrice)}</span>
                </div>
                {selectedProduct.stockState === 'sold-out' ? (
